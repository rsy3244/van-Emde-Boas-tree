...
\begin{frame}\frametitle{vEB木 計算量\uniqnum{3}}
    \onslide*<1>{
        \begin{block}{$\Member(V,x)$}
            関数内での再帰呼び出しは$1$回以下なので, 時間計算量は \structure{$O(\Func{loglog}u)$}
        \end{block}
    %}
    ...
    %\onslide*<3>{
        \begin{block}{$\Successor(V,x)$}
			...
			$\Successor(V,x)$の時間計算量は\structure{$O(\Func{loglog}u)$}
        \end{block}
    }
	...
	\onslide*<2>{
        \begin{block}{$\Insert(V,x)$}
			...
			よって, $\Insert(V,x)$の時間計算量は\structure{$O(\Func{loglog}u)$}
	...
	\onslide*<3>{
        \begin{block}{$\Delete(V,x)$}
			...
			よって, $\Delete(V,x)$の時間計算量は\structure{$O(\Func{loglog}u)$}
        \end{block}
    }
\end{frame}
\end{document}
